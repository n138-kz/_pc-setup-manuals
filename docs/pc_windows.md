...
> を新規作成する。  
> ドライバの見分け方は、  
> コントロールパネル→ネットワークとインターネット→ネットワーク接続 を開いて(コマンドで開く場合は `control netconnections`)、対象の NIC のプロパティを開き、  
> その「構成」から「ドライバタブ」を開いて「ドライバファイルの詳細」を開くとドライバファイルの一覧が出てくるので、そこのファイル名から判別
